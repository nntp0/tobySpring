...
public class DITest {	
	...
	public void isSingletonObject() {
		...
		assertEquals(dao1, CoreMatchers.is(CoreMatchers.sameInstance(dao2)));
	}
	@Test
	public void isSameAutowiredAndGetBean() throws SQLException {
		...
		assertEquals(dao, CoreMatchers.is(CoreMatchers.sameInstance(dao2)));
	}
	@Test
	public void isStringInjected() {
		assertEquals(user.getId(), CoreMatchers.is("qweshjkd"));
		assertEquals(user.getName(), CoreMatchers.is("저기에요"));
		assertEquals(user.getPassword(), CoreMatchers.is("2255"));
	}
}